...
class Scene:
    ...
    def __init__(
        ...
        if renderer is None:
            self.renderer = CairoRenderer(
                camera_class=self.camera_class, skip_animations=self.skip_animations,
            )
        else:
        ...
        def add_safe_mobjects_from_list(list_to_examine, set_to_remove):
            for mob in list_to_examine:
                if mob in set_to_remove:
                    continue
                intersect = list(set_to_remove.intersection(mob.get_family()))
                if intersect:
                    if len(intersect) == 1:
                        mob.remove(intersect[0])
                    else:
    ...
    def get_moving_and_static_mobjects(self, animations):
        ...
        all_moving_mobject_families = extract_mobject_family_members(
            moving_mobjects, use_z_index=self.renderer.camera.use_z_index,
        )
    ...
    def interactive_embed(self):
        ...
        keyboard_thread = threading.Thread(
            target=ipython, args=(shell, local_namespace),
        )
