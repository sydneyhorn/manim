...
from manim import Circle, FadeIn, Mobject, Scene, Square, print_family, tempconfig
...
def test_scene_add_remove_for_subobjects():
    with tempconfig({"dry_run": True}):
        scene = Scene()
        assert len(scene.mobjects) == 0
        square = Square()
        ...
        # Checks that the parent object was added to the scene
        scene.add(square)
        assert len(scene.mobjects) == 1

        # Checks to see whether the scene removes and adds a new subobject to the parent object
        scene.remove(circle)
        scene.add(circle2)
        # print_family(square)
        assert len(scene.mobjects) == 1
